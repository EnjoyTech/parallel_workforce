...
RSpec.shared_examples 'perform' do |job_method, time_zone_name=nil, locale=nil|
  ...
  context 'when job executed after job_key_expiration' do
    let(:execution_time) { Time.now + ::ParallelWorkforce.configuration.job_key_expiration + 1 }

    it 'raises error due to serialize_actor_args being unavailable' do
      expect { subject }.to raise_error("Unable to locate serialized data required for Performer")
    end
  end
end
